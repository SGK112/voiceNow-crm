...
import UniversalAIBuilder from '../components/UniversalAIBuilder';
import ReactFlow, {
  Background,
  Controls,
  MiniMap,
...
import CredentialConnectionModal from '../components/CredentialConnectionModal';
...
// Available node templates - organized by category (CRM-focused)
const nodeTemplates = [
  // === TRIGGERS ===
  ...
  { type: 'new_lead', label: 'New Lead Trigger', icon: '👤', color: '#10b981', description: 'On lead creation', category: 'Triggers' },
  { type: 'lead_stage_change', label: 'Lead Stage Change', icon: '🔄', color: '#10b981', description: 'Stage updated', category: 'Triggers' },
  { type: 'deal_created', label: 'Deal Created', icon: '💼', color: '#10b981', description: 'New deal trigger', category: 'Triggers' },
  ...
  // === CRM ACTIONS ===
  { type: 'save_lead', label: 'Create Lead', icon: '📝', color: '#3b82f6', description: 'Save to CRM', category: 'CRM' },
  { type: 'update_lead', label: 'Update Lead', icon: '✏️', color: '#3b82f6', description: 'Modify lead data', category: 'CRM' },
  { type: 'update_contact', label: 'Update Contact', icon: '✏️', color: '#3b82f6', description: 'Update CRM data', category: 'CRM' },
  { type: 'create_deal', label: 'Create Deal', icon: '💰', color: '#3b82f6', description: 'New opportunity', category: 'CRM' },
  { type: 'move_stage', label: 'Move Stage', icon: '➡️', color: '#3b82f6', description: 'Change pipeline', category: 'CRM' },
  { type: 'assign_to', label: 'Assign To', icon: '👥', color: '#3b82f6', description: 'Assign lead/deal', category: 'CRM' },
  { type: 'add_note', label: 'Add Note', icon: '📋', color: '#3b82f6', description: 'Add activity note', category: 'CRM' },
  { type: 'add_tag', label: 'Add Tag', icon: '🏷️', color: '#3b82f6', description: 'Tag lead/deal', category: 'CRM' },
...
function WorkflowStudioContent() {
  ...
  const saveWorkflow = async () => {
    ...
    try {
      ...
      await api.put(`/crm-workflows/${currentWorkflow._id}`, {
  ...
  const deleteWorkflow = async (workflowId) => {
    ...
    try {
      await api.delete(`/crm-workflows/${workflowId}`);
      fetchWorkflows();
  ...
  const toggleWorkflow = async (workflowId, currentState) => {
    try {
      const endpoint = currentState ? `/crm-workflows/${workflowId}/deactivate` : `/crm-workflows/${workflowId}/activate`;
